...
### Improved
- **Localization**: Standardized terminology across all languages (English, Spanish, French, Japanese, Korean, Vietnamese) to be more professional and widely understood. Replaced "Alchemist", "Essence", "Transmute" with standard terms like "System", "Database", "Generate".

## [1.0.85] - 2026-01-30

### Improved
- **Localization**: Polished Spanish and French translations to better align with "Alchemist" branding and technical terminology. Consistent capitalization and tone across dashboard and settings.
...
### Documentation
- **Browser Support**: Updated documentation to explicitly list support for Vivaldi, Opera, Opera GX, and Chromium.
- **System Logs**: Added comprehensive documentation for the System Logs feature, including signal management and error troubleshooting.
- **Guides**: Enhanced "Getting Started" and "Configuration" guides with clearer steps and updated screenshots references.

### Improved
- **Localization**: Refined UI text and translations across all supported languages (English, Spanish, French, Japanese, Korean, Vietnamese) for better clarity and consistency in the Setup Wizard and Account Settings.

## [1.0.82] - 2026-01-30

### Added
- **UI**: Added visibility toggles (Eye/EyeOff icons) for all sensitive input fields, including Passwords, API Keys, and Access Tokens, across Authentication, Setup Wizard, and Account Settings screens.

## [1.0.81] - 2026-01-29

### Refactored
- **TTS System**: Migrated Text-to-Speech logic to a global `TTSContext`. This resolves issues with overlapping audio playback and ensures a singleton audio controller across the entire application.
- **State Management**: Introduced `speakingId` tracking to correctly visualize which specific message is currently speaking in the chat interface.

## [1.0.80] - 2026-01-29

### Improved
- **Build**: Fully synchronized `package-lock.json` with the published `@realtimex/sdk` registry version, removing residual local link references.

## [1.0.79] - 2026-01-29

### Fixed
- **Dependencies**: Switched `@realtimex/sdk` dependency from a local file path to the published registry version (`^1.2.4`), ensuring reliable installs in production environments.

## [1.0.78] - 2026-01-29

### Improved
- **Build**: Updated `tsconfig` to use `node16` module resolution. This resolves ESM/CJS interop issues and aligns the API build process with modern Node.js standards.
- **SDK**: Enhanced type safety for `SDKService` by explicitly typing provider responses.
...
### Improved
- **UX**: The **Live Terminal** now opens immediately when a sync is triggered, providing instant visual feedback.
- **UI**: Polished the "Stop Sync" button with a centered, pill-shaped design and smoother entry animations.
...
### Improved
- **UI**: Standardized the **Context Sidebar** styling to match the rest of the Chat Interface. Updated border opacity and corner radius to align with the new design system, ensuring a cohesive look when reviewing RAG citations.
...
### Fixed
- **Localization**: Restored missing translation keys (`copied_json`, `json_footer`) for the Transmute Engine JSON viewer across all supported languages.
- **UI**: Fixed a minor styling inconsistency in the JSON viewer's copy button.
...
### Improved
- **UI**: Polished the **Language Switcher** with the new design system tokens, ensuring consistent backgrounds and borders across themes.
- **UX**: Fixed the dropdown animation direction when the switcher is positioned at the bottom of the sidebar (now correctly slides up).
...
### Documentation
- **Localization**: Added multi-language documentation for core topics in Japanese, French, Vietnamese, Spanish, and Korean.

### Fixed
- **UI**: Resolved a `ReferenceError` that occurred when interacting with signals in the discovery view.
