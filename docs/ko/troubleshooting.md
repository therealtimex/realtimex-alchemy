...
-   **"Failed to connect to Supabase"**: 인터넷 연결을 확인하고 설정 마법사나 `.env` 파일에 있는 **Supabase URL** 및 **Anon Public Key**를 확인하세요. 일반 연결에 Service Role Key를 사용하지 마세요.
-   **"Table 'xyz' does not exist"**: 마이그레이션을 누락했을 수 있습니다. 설정 마법사로 이동하여 **"Run Migrations"**를 다시 클릭하세요. 이 단계에서는 **Supabase Access Token**이 필요합니다.
...
> [!CAUTION]
> 공개 포럼이나 문제 보고서에 Supabase Access Token 또는 API 키를 절대 공유하지 마세요.
