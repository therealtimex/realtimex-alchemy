...
-   **프로필**: 이름과 아바타를 관리합니다.
-   **Supabase 연결**: 데이터베이스를 이동한 경우 **Supabase URL**과 **Anon Public Key**를 업데이트하세요.
-   **데이터베이스 마이그레이션**: 설정 마법사를 통해 스키마를 업데이트할 때 **Supabase Access Token**을 입력하라는 메시지가 표시됩니다.
