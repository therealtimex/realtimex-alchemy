...
export function MessageBubble({ message, isLastMessage, autoSpeak }: MessageBubbleProps) {
    ...
    const markdownComponents: Partial<Components> = {
        ...
        blockquote: ({ ...props }) => <blockquote className="border-l-2 border-border/50 pl-3 text-fg/70 italic mb-2" {...props} />,
        table: ({ ...props }) => <table className="w-full text-xs border-collapse my-2" {...props} />,
        th: ({ ...props }) => <th className="border border-border/20 bg-surface/50 px-2 py-1 text-left font-semibold" {...props} />,
        td: ({ ...props }) => <td className="border border-border/20 px-2 py-1 align-top" {...props} />,
